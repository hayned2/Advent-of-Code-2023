...
# Parse input file
input = open(os.path.dirname(os.path.realpath(__file__)) + '/day24_input.txt')
...
for x in range(len(sorted_keys)):
    for y in range(x + 1, len(sorted_keys)):
        intersection = calculate_intersection(sorted_keys[x], points[sorted_keys[x]], sorted_keys[y], points[sorted_keys[y]])

        # If they intersect, check if it happens within our desired test area
        if intersection is not None and test_area[0] <= intersection[0] <= test_area[1] and test_area[0] <= intersection[1] <= test_area[1]:
...
for s in range(len(sorted_keys)):
    ...
    for dimension in range(3):
        ...
        solver.add(starting_position[dimension] + (t * god_throw_velocity[dimension]) == position[dimension] + (t * velocity[dimension]))
...
if solver.check() == sat:
    ...
    print(f"By throwing a rock from ({', '.join([str(solver.model().evaluate(starting_position[dimension])) for dimension in range(3)])}) with a velocity of ({', '.join([str(solver.model().evaluate(god_throw_velocity[dimension])) for dimension in range(3)])}), we can hit all rocks, giving us a coordinate sum of:", solution)
