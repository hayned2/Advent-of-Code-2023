...
# Parse input file
input = open(os.path.dirname(os.path.realpath(__file__)) + '/day13_input.txt')
...
def search_for_smudges(current_pattern, old_reflection_value):
    for row in range(len(current_pattern)):
        for column in range(len(current_pattern[row])):
            smudged_row = current_pattern[row][:column] + ('.' if current_pattern[row][column] == '#' else '#') + current_pattern[row][column + 1:]
            result = search_for_reflection(current_pattern[:row] + [smudged_row] + current_pattern[row + 1:], old_reflection_value)
