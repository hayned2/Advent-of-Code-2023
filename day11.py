...
def manhattan_distance(galaxy1, galaxy2, columns_without_galaxies, rows_without_galaxies):
    ...
    empty_lines_crossed = len(spanned_rows.intersection(rows_without_galaxies)) + len(spanned_columns.intersection(columns_without_galaxies))
...
for x in range(len(galaxies)):
    for y in range(x + 1, len(galaxies)):
            distance1, distance2 = manhattan_distance(galaxies[x], galaxies[y], columns_without_galaxies, rows_without_galaxies)
            distances += distance1
...
print("The sum of the shortest paths between all pairs of galaxies, where empty lines count as 1,000,000 units is:", distances2)