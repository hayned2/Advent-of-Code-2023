...
# Parse input file
input = open(os.path.dirname(os.path.realpath(__file__)) + '/day16_input.txt')
...
def shoot_beam(beams):
    ...
    while len(beams) > 0:
        ...
        for direction in next_direction:
            ...
            if next_position[0] >= 0 and next_position[1] >= 0 and next_position[0] < height and next_position[1] < width:
...
for x in range(height):
    ...
    # Part 1 Solution
    if x == 0:
        print("By shooting the laser at the top-left of the board going right, the total number of tiles energized is:", energized)
    most_energized = max(most_energized, energized)
