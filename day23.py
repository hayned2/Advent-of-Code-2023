...
# Parse input file
input = open(os.path.dirname(os.path.realpath(__file__)) + '/day23_input.txt')
    ...
    for direction in slopes.values():
        ...
        if neighbor in graph1:
            ...
            if spot in slopes.keys():
                node.forced_neighbor = (node.initial_position[0] + slopes[spot][0], node.initial_position[1] + slopes[spot][1])
...
print("The longest path down the mountain with the slippery tiles is:", find_longest_path(starting_node1, graph1, False))
print("The longest path down the mountain without the slippery tiles is:", find_longest_path(starting_node2, graph2, True))
